...
import { Input, Rate, Button, Comment, Avatar, message } from 'antd';
...
const Appraises = (props) => {
  ...
  const getList = useCallback(() => {
    getAppraiseList({ params: { courseId: props.data.id } }).then((res) => {
      if (res.data.code === 0) {
  ...
  return (
    <>
      <div className="appraise-editor">
        ...
        {list.length > 0 ? <AppraisesList list={list} /> : <div className="appraise-no"> 暂无评价， 快来抢沙发吧！ </div>}
      </div>
    </>
  );
};

const AppraisesList = ({ list }) => {
